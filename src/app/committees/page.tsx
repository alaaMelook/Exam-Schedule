...
import { Plus, Building2, Pencil, Trash2, X, Check, Calendar, Upload } from 'lucide-react'
...
export default function CommitteesPage() {
  ...
  function openEdit(c: Committee) {
    setForm({
      name: c.name, college: c.college, location: c.location || '',
      exam_date: c.exam_date, start_time: c.start_time, end_time: c.end_time,
      main_observers: c.main_observers, backup_observers: c.backup_observers
    })
    setEditingId(c.id)
    setShowModal(true)
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm('هل تريد حذف هذه اللجنة؟ سيتم حذف جميع التكليفات المرتبطة بها.')) return
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Building2 className="w-6 h-6 text-purple-600" /> اللجان
            </h1>
            <p className="text-gray-500 text-sm mt-1">{committees.length} لجنة مسجلة</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
              ...
              <div key={date} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="bg-gradient-to-l from-purple-50 to-indigo-50 px-6 py-3 border-b border-purple-100 flex items-center gap-2">
                  ...
                  <span className="font-bold text-purple-800">
                    {getArabicDay(date)} — {formatDate(date)}
                  </span>
                  <span className="text-purple-500 text-sm mr-1">({comms.length} لجنة)</span>
                </div>
                  ...
                  <tbody>
                    {comms.map(c => (
                      <tr key={c.id}>
                        ...
                        <td>
                          <span className="badge-main">{c.main_observers}</span>
                        </td>
                        <td>
                          <span className="badge-backup">{c.backup_observers}</span>
                        </td>
                        <td>
                          <div className="flex items-center gap-2">
                            <button onClick={() => openEdit(c)} className="text-purple-500 hover:text-purple-700 p-1">
            ...
            {committees.length === 0 && (
              <div className="bg-white rounded-2xl p-12 text-center text-gray-400">لا توجد لجان</div>
            )}
          </div>
        )}
      </main>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white">
              <h2 className="font-bold text-lg text-gray-900">
                {editingId ? 'تعديل لجنة' : 'إضافة لجنة جديدة'}
              </h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">اسم اللجنة / الفرع *</label>
                  <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                    className="input-field" placeholder="مثال: مدرج 1 يحيى" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">الكلية *</label>
                  <input type="text" value={form.college} onChange={e => setForm({...form, college: e.target.value})}
                    className="input-field" placeholder="مثال: كلية العلوم" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">المكان</label>
                  <input type="text" value={form.location} onChange={e => setForm({...form, location: e.target.value})}
                    className="input-field" placeholder="مثال: مبنى أ" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">تاريخ الامتحان *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">عدد الملاحظين الأساسيين</label>
                  <input type="number" min={1} max={10} value={form.main_observers}
                    onChange={e => setForm({...form, main_observers: parseInt(e.target.value)})}
                    className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">عدد الملاحظين الاحتياطيين</label>
                  <input type="number" min={0} max={10} value={form.backup_observers}
                    onChange={e => setForm({...form, backup_observers: parseInt(e.target.value)})}
                    className="input-field" />
                </div>
              </div>
            </div>
